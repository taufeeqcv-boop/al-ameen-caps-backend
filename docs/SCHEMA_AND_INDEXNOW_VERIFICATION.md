...
- **Live check:** After deploy, open `https://www.alameencaps.com/4ed08a1ef99a4e2c900e1f0cd2eb69f8.txt`. You should see only the key string (plain page). Ensure `INDEXNOW_KEY` is set in Netlify (Build & deploy → Environment) so the file is created on each build.
